...
function ChainAssetSelector(
  ...
  return (
    <div className="mx-auto flex h-full w-full max-w-[400px] flex-col justify-center gap-6 rounded-lg bg-[#1A1A1A] p-8">
      <h3 className="text-xl font-bold text-white">Customize the Widget</h3>

      <div className="flex flex-col gap-5">
        <div className="flex flex-col gap-3">
          <label htmlFor="chain-select" className="text-gray-300">
            Target Chain
          </label>
          <div className="relative">
            <select
              ...
              className="w-full appearance-none rounded-lg border border-[#3A3A3A] bg-[#2A2A2A] px-4 py-3 text-white"
            >
              {CHAINS.map((chain) => {
                return (
        ...
        <div className="flex flex-col gap-3">
          ...
          <input
            ...
            className="w-full rounded-lg border border-[#3A3A3A] bg-[#2A2A2A] px-4 py-3 text-white"
          />
          {validationMessage && (
            <div
              className={`text-sm ${isAddressValid ? "text-green-500" : "text-red-500"}`}
            >
              {validationMessage}
            </div>
          )}
        ...
        {isAddressValid && (
          <div className="mt-2 flex flex-col gap-2">
            ...
            <div className="overflow-x-auto break-all rounded bg-[#2A2A2A] p-2 text-xs text-gray-100">
              {destinationAsset}
            </div>
          </div>
        )}
      ...
      <button
        className={`mt-4 rounded-lg px-6 py-3 font-normal ${
          isAddressValid
            ...
            : "cursor-not-allowed bg-[#4A4A4A] text-[#A0A0A0]"
        }`}
