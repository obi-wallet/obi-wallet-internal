...
import { BitcoinTargetChain } from "./index";
...
export class BitcoinMpcSigner {
  ...
  public static async fromWallet(
    ...
    return new BitcoinMpcSigner(wallet, targetChain, publicKey);
  }
  ...
  public async signTransaction(psbtBase64: string): Promise<string> {
    ...
    for (let i = 0; i < psbt.inputCount; i++) {
      ...
      if (input.witnessUtxo) {
        ...
        const nonWitnessTx = bitcoinjs.Transaction.fromBuffer(input.nonWitnessUtxo);
        const prevOut = nonWitnessTx.outs[txInput.index];
        if (!prevOut) {
          throw new Error(`Failed to retrieve previous output at index ${txInput.index}`);
        }
      ...
      const signatureWithHashType = Buffer.concat([signature, Buffer.from([bitcoinjs.Transaction.SIGHASH_ALL])]);
  ...
  private assembleDERSignature(r: Buffer, s: Buffer): Buffer {
    ...
    const derEncoded = Buffer.concat([
      Buffer.from([0x30]),
      Buffer.from([rPrefix.length + rTrimmed.length + sPrefix.length + sTrimmed.length + 4]),
      Buffer.from([0x02]),
  ...
  private getCompressedPublicKey(): Buffer {
    ...
    if (publicKeyBuffer.length === 33) {
      ...
      const compressedPublicKey = secp256k1.publicKeyConvert(publicKeyBuffer, true);
  ...
  public addIntentionsResults(params: {
    ...
    this.mpcSigner.addIntentionsResults(params);
  }
