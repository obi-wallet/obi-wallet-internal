...
import {
  AbstractTargetChain,
  Caip19Asset,
  AssetInfo as BaseAssetInfo,
} from "@obi-wallet/sdk-abstract-target-chain";
import { Secp256k1PublicKey } from "@obi-wallet/sdk-secp256k1";
import { SessionRequestResponse } from "@obi-wallet/wallet-connect";
...
export class BitcoinTargetChain extends AbstractTargetChain<string, string> {
   ...
   */
  async obiAccountAddressQueryFn(publicKeys: {
    secp256k1?: Secp256k1PublicKey;
  }): Promise<string> {
    if (!publicKeys.secp256k1) {
      throw new Error("Secp256k1 public key required for Bitcoin addresses");
    }
   ...
   */
  async tokenBalanceQueryFn(_params: {
    address: string;
  ...
  ): Promise<BaseAssetInfo | null> {
    if (
      id.toUpperCase() === "BTC" ||
      id.toLowerCase() === "bip122:000000000019d6689c085ae165831e93/slip44:0"
    ) {
      return {
  ...
  /**
   * Handles a WalletConnect session request.
   * @param payload - The WalletConnect session request payload.
   * @returns A promise that resolves to the session response.
   */
  public async handleWalletConnectSessionRequest(_payload: {
    ...
    return {
      error: {
        code: -32601,
        message: "WalletConnect is not supported for Bitcoin.",
      },
    };
  }
   ...
   */
  public isTokenAsset(
    _assetId: `${string}:${string}/${string}:${string}`,
  ): boolean {
