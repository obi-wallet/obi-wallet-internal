...
export async function fetchPrices({ chainId }: { chainId: CosmosChain }) {
  return await withCosmosCosmWasmClient(chainId, async (cosmWasmClient) => {
    const denoms = (() => {
      switch (chainId) {
        case "uni-3":
          return ["ujuno"];
    ...
    const getUsdRate = async (denom: string) => {
      ...
      try {
        const basePriceInUsdElements = await cosmWasmClient.queryContractSmart(
          route[1],
          {
            reverse_simulation: {
              ask_asset: {
