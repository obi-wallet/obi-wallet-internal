...
interface PhoneNumberBottomSheetContentProps {
  payload: TerraMultisig["phoneNumber"];
  wallet: TerraMultisigWallet;
  multisig: TerraMultisig;

  getTransactionInformation(): Promise<{
    transaction: Tx;
    signDoc: SignDoc;
  }>;

  onSuccess(signature: SignatureV2): void;
}

const PhoneNumberBottomSheetContent =
  observer<PhoneNumberBottomSheetContentProps>(
    ({ payload, wallet, multisig, getTransactionInformation, onSuccess }) => {
      const { chainStore } = useStore();
      const chainId = chainStore.currentTerraChain;
      const intl = useIntl();
